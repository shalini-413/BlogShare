...
import { getQuotes, deleteQuote, deleteComment } from "../services/api";
...
export default function Profile() {
  ...
  const fetchMyQuotes = async () => {
    ...
    const enhancedQuotes = await Promise.all(
      res.data
      .filter((q) => {
        const quoteUserId = typeof q.user === "string" ? q.user : q.user?._id;
        ...
        .map(async (quote) => {
          ...
          return { ...quote, comments: enhancedComments };
        })
    );

    setMyQuotes(enhancedQuotes);
  };
  ...
  return (
    <div className="min-h-screen bg-black px-4 sm:px-6 py-12 md:py-16 transition-all duration-500">
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <h2 className="text-2xl sm:text-2xl font-bold text-left text-white mb-8 border-l-4 border-orange-400 pl-4 py-2">
          📝{" "}My Blogs
        </h2>
        ...
        {myQuotes.some((q) => q.flagged) && (
          <div className="bg-red-100 border border-red-300 text-red-700 p-4 rounded-md mb-6 text-center font-medium">
            ⚠️ {myQuotes.filter((q) => q.flagged).length} of your blog post(s) flagged as{" "}
          ...
          <div className="grid grid-cols-1 gap-6">
            {myQuotes.map((q, index) => {
              ...
              return (
                ...
                >
                  {/* ✅ Blog Title */}
                  <h3 className="text-3xl md:text-4xl font-extrabold text-center uppercase tracking-wider text-white mb-6 border-b-4 border-orange-400 inline-block pb-2 px-4 shadow-sm mx-auto">
